...
function mainPage(){
	...
	switch (graphSettings.mainPage){
		case 'graph':
			var pageContent = `<div id="cy" class="px-1"></div>`
...
// Ensure Cytoscape resizes when the drawer opens/closes
document.addEventListener('shown.bs.offcanvas', () => window.cy?.resize());
document.addEventListener('hidden.bs.offcanvas', () => window.cy?.resize());
